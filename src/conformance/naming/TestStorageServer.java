...
class TestStorageServer implements Storage, Command
{
    ...
    {
        this.test = test;
        client_skeleton = new StorageSkeleton("localhost", 9500);
        command_skeleton = new CommandSkeleton("localhost", 9501);
        skeletons_started = false;
    }
    
    TestStorageServer(Test test, String ip, int port1, int port2)
    {
    	this.test = test;
    	client_skeleton = new StorageSkeleton(ip, port1);
    	command_skeleton = new CommandSkeleton(ip, port2);
    	skeletons_started = false;
    }
    ...
    private class StorageSkeleton extends Skeleton<Storage>
    {
        ...
        //
        StorageSkeleton(String ip, int port)
        {
        	super(Storage.class, TestStorageServer.this, new InetSocketAddress(ip, port));
        }
    ...
    private class CommandSkeleton extends Skeleton<Command>
    {
        ...
        //
        CommandSkeleton(String ip, int port)
        {
        	super(Command.class, TestStorageServer.this, new InetSocketAddress(ip, port));
        }
